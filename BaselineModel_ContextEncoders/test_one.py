...
imbefore = utils.load_image(opt.test_image, opt.imageSize)
...
imbefore = utils.load_image(opt.test_image, opt.imageSize)
imafter = utils.load_image("val_recon_samples.png",opt.imageSize)
imb =plt.imshow(imbefore)
plt.show(imb)
ima = plt.imshow(imafter)
plt.show(ima)
imbefore = plt.imread(opt.test_image).astype(numpy.float32)[32:32+64,32:32+64,:]
imafter = plt.imread('val_recon_samples.png').astype(numpy.float32)[32:32+64,32:32+64,:]


print(psnr(imbefore,imafter))

print(errG, file=open("output.txt", "a"))
