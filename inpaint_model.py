...
class Inpaint:
    def __init__(self):
        ...
        filename = "/Users/gammu/Documents/study/Advanced machine learning/Project/uic-cs512-project/checkpoint/saved_model.pth"
    ...
    def generate_z_hat(self,real_images, images, masks):
        ...
        for i in range(self.nIters):
            ...
            if i%250 == 0:
                with torch.no_grad():
                    channeled_masks = torch.empty(masks.shape[0],3,masks.shape[1],masks.shape[2])
                    unsq_masks = torch.unsqueeze(masks,1)
                    for j in range(len(masks)):
                        channeled_masks[j] = torch.repeat_interleave(unsq_masks[j], 3, dim = 0)
